...
public class PaintHouseSolution {
    ...
    public int minCost(int n, int[] redCosts, int[] blueCosts, int[] greenCosts) {
    	...
    	// 用二维数组保存第i+1间房子选择第k种颜色时的最少花费(k=0表示选择红色、1选择蓝色、2选择绿色)。
    	int Price[][] = new int[n][3];
    	// 初始化只有一间房子需要粉刷时选择各种颜色所需要的最小花费。
    	Price[0][0] = redCosts[0];
    	Price[0][1] = blueCosts[0];
    	Price[0][2] = greenCosts[0];
    	for(int i = 1; i < n; i++){
    		// 第i+1间房子粉刷红色所对应的最小花费为第i+1间房子粉刷为红色的花费+第i间房子粉刷其他颜色所需的最小花费。
    		Price[i][0] = redCosts[i] + Math.min(Price[i - 1][1], Price[i - 1][2]);
    		Price[i][1] = blueCosts[i] + Math.min(Price[i - 1][0], Price[i - 1][2]);
    		Price[i][2] = greenCosts[i] + Math.min(Price[i - 1][0], Price[i - 1][1]);
    	}
